...
Example of doing pagination:
    ...
    var  expr = soar.sql('Person'),
         cmd = {
            op: 'list',
            expr: expr,
            range: soar.range(1, 10)    // return the first page with page size set to 10
         };
...
Sometimes dumping SQLs of every database query could be overwhelming. You can choose to print a specific query by setting the 'debug' property of a **soar** command to true (this feature only works for the _execute()_ function). For example:
