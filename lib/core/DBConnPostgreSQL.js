...
class DBConnPostgreSQL extends DBConn {
    ...
    constructor(dbConfig)  {
        ...
        if (dbConfig.pool)  {
            // Use pre-created pool (e.g., PGlite adapter for in-memory testing)
            ...
            delete  pgConfig.searchPath;
            delete  pgConfig.role;
    ...
    async connect()  {
        ...
        client._dbType = 'postgresql';

        // Set schema search path and role per connection checkout.
        // This ensures correct behavior with PgBouncer (transaction mode)
        // where DISCARD ALL resets session state between clients.
        if (this.dbConfig.searchPath)
            await client.query(`SET search_path TO ${this.dbConfig.searchPath}`);
        if (this.dbConfig.role)
            await client.query(`SET ROLE ${this.dbConfig.role}`);
