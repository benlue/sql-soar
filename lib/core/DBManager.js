...
let  useDB = {},
     formulaMap = {},
     _dftName,
     _prevOptions;      // previous initialization options in string form
...
exports.init = function(options)  {
    ...
    // skip re-init if options are identical to previous settings
    try  {
        let  optStr = JSON.stringify(options);
        if (_prevOptions === optStr)
            return;
        _prevOptions = optStr;
    }
    catch(e)  {
        // options may contain non-serializable objects (e.g. pool instances)
        _prevOptions = null;
    }

    // destroy the old pools if soar is being re-initialized
...
exports.getDatabaseType = function(dbName)  {
    const  name = dbName || _dftName;
    return  useDB[name]  ?  useDB[name].databaseType : undefined;
}
...
exports.invalidateSchemaCache = function(tableName)  {
    for (let name in useDB)
        useDB[name].invalidateSchemaCache(tableName);
}
