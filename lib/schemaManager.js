...
*/
var  mysql = require('./sql/sqlGenMySql.js'),
     util = require('util');
...
exports.alterTable = function(conn, schema, cb)  {
    ...
    if (updSchema.add && updSchema.add.column)
        updSchema.add.column = convertColSpec(updSchema.add.column);
    if (updSchema.alter && updSchema.alter.column)
        updSchema.alter.column = convertColSpec(updSchema.alter.column);
...
function  convertColSpec(cols)  {
    var columns = [];
    for (var key in cols)  {
        var  prop = cols[key],
             c = {title: key, name: prop.title, type: toSQLType(prop)};
        if (prop.options)
            c.options = prop.options;
...
exports.renameTable = function(conn, oldName, newName, cb)  {
    var  sql = util.format('RENAME TABLE `%s` TO `%s`;', oldName, newName);
    try  {
        conn.query(sql, function(err) {
            cb(err);
        });
