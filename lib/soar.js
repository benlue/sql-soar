...
function  normalizeResult(result)  {
	if (Array.isArray(result))
		return  result[0];             // MySQL: [rows, fields] -> rows
	if (result && result.rows)
		return  result.rows;           // PostgreSQL: {rows, ...} -> rows
	return  result;
}
...
exports.createTable = async function(conn, schema)  {
	...
	try  {
		return  await getSchemaManager(conn._dbType).createTable(conn, schema);
	}
	finally  {
...
exports.alterTable = async function(conn, schema)  {
	...
	try  {
		const  result = await getSchemaManager(conn._dbType).alterTable(conn, schema);
		if (schema.title)
			dbm.invalidateSchemaCache(schema.title);
		return  result;
	}
	finally  {
		if (needRelease)  conn.release();
...
exports.deleteTable = async function(conn, tbName)  {
	...
	try  {
		const  result = await getSchemaManager(conn._dbType).deleteTable(conn, tbName);
		dbm.invalidateSchemaCache(tbName);
		return  result;
	}
	finally  {
		if (needRelease)  conn.release();
...
exports.renameTable = async function(conn, oldName, newName)  {
	...
	try  {
		const  result = await getSchemaManager(conn._dbType).renameTable(conn, oldName, newName);
		dbm.invalidateSchemaCache(oldName);
		dbm.invalidateSchemaCache(newName);
		return  result;
	}
	finally  {
		if (needRelease)  conn.release();
...
exports.describeTable = async function(conn, tbName)  {
	...
	try  {
		return  await getSchemaManager(conn._dbType).describeTable(conn, tbName);
	}
	finally  {
		if (needRelease)  conn.release();
	}
 ...
 * Call this after modifying a table schema via raw SQL.
 */
exports.invalidateSchemaCache = function(tableName)  {
	dbm.invalidateSchemaCache(tableName);
};


function  getSchemaManager(dbType) {
	dbType = dbType || dbm.getDB().databaseType;
	return  dbType === 'postgresql' ?  new SchemaManagerPostgreSQL() : new SchemaManagerMySQL();
}
