...
function  configDB(options)  {
	...
	var  dbName = dbConfig.alias || dbConfig.database;
...
exports.execute = function(cmd, data, query, handler)  {
	...
	if (!ncmd.expr.columns || !ncmd.expr.filters)
		...
		autoFillSchema(dbName, tbName, ncmd, data, query, handler);
	else
		runTemplate({dbName: dbName, tbName: tbName}, ncmd, data, query, handler);
};
				...
				if (err)
					...
					if (options.op === 'insert')
						...
						if (expr._count && sameQuery(expr._query, query))
							...
							sql = getSqlGenerator().toSQL(options);
