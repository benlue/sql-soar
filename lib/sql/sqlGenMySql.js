...
* authors: Ben Lue
* license: MIT License
* Copyright(c) 2015 ~ 2020 Gocharm Inc.
*/
...
function genList(expr, q, p, range, fields, debug)  {
	let  sql = composeQ( expr, q, p, fields, debug )
...
function  genListCount(expr, q, p)  {
	return  composeQ( expr, q, p, 'COUNT(*) AS ct', false, true )
...
function composeQ(expr, q, p, fld, debug, dontCache)  {
			...
			if (Array.isArray(fld))
				...
				fields.forEach( function(f)  {
					if (fld.indexOf(f) >= 0)  {
						...
						sql += KEY_LIST.includes(f)  ?  ('`' + f + '`') : f
					}
				})
			else
				sql += KEY_LIST.includes(fld)  ?  ('`' + fld + '`') : fld
		}
		else
			fields.forEach( function(f) {
				if (notFirst)
...
function  matchFilter(filter, q, p)  {
	...
	if (op === 'AND' || op === 'and' || op === 'OR' || op === 'or')  {
		...
		if (q.hasOwnProperty(fname))  {
			const  colName = filter.field || filter.name
			sql = KEY_LIST.includes(colName)  ?  '`' + colName + '`' : colName
